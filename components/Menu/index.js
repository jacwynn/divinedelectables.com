            ...
            <div className={`container mx-auto bg-brand-bg-grey mt-[6.25rem] rounded-[2.5rem] p-[2.5rem] lg:p-16 lg:px-0`}>
                <div className="space-x-8 text-center">
                    {Object.keys(categories).map(category => (
                        categories[category].length > 0 && <button key={category} onClick={() => setActiveTab(category)} className={`p-4 mt-8 text-white text-[1.75rem] uppercase font-bold ${activeTab === category ? 'bg-brand-red' : 'text-brand-red'} hover:bg-brand-red hover:text-white`}>{category}</button>
                    ))}
                </div>
